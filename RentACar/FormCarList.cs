...
namespace RentACar
{
    public partial class FormCarList : Form
    {
        ...
        private void mnuDelete_Click(object sender, EventArgs e)
        {
            ...
            DialogResult res = MessageBox.Show("Czy na pewno usunąć rekord?", "Pytanie", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void RefreshData()
        {
            ...
            string sql = @"SELECT c.id, b.name AS brand, m.name AS model, t.name AS car_type, 
                        c.registration_plate, c.engine, c.manufacturer_year, c.avail, c.fuel

                        FROM cars AS c, car_types AS t, car_models AS m, car_brands AS b
                        WHERE c.type_id = t.id and c.model_id = m.id AND m.brand_id=b.id";
            adapter.SelectCommand = new MySqlCommand(sql, GlobalData.connection);

            DataTable dt = new DataTable();
            adapter.Fill(dt);

            // przypinanie danych uzyskanych z MariaDB do grida
